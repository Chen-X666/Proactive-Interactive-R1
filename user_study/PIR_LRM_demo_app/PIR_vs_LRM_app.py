...
from utils.math_model_client import ModelClient
...
base_lrm_model_url = "http://localhost:1138"
base_lrm_model_name = "DeepSeek-R1-Distill-Qwen-7B"
...
MODEL_CONFIGS = {
    ...
    "lrm": dict(
        model_path=base_lrm_model_name,
        base_url=base_lrm_model_url,
...
def render_thinking_indicator(label):
    ...
    <div class="thinking-box">
        ...
        <div>
            ...
            <div class="thinking-desc">The model is reasoning through the problem. This may take a moment.</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def _extract_output(client):
    if not client or not client.completion:
        return None
    return client.completion.split('</think>')[-1].strip() if "</think>" in client.completion else client.completion.strip()
...
def finalize_output(key):
    ...
    if not client or not client.completion:
        ...
        if "</think>" in client.completion:
            think_match = re.search(r"<think>(.*?)</think>", client.completion, re.DOTALL)
            last_reasoning_idx = -1
            for i, m in enumerate(messages):
                if m['role'] == 'assistant_reasoning':
                    last_reasoning_idx = i
            last_user_idx = -1
            for i, m in enumerate(messages):
                if m['role'] == 'user':
                    last_user_idx = i
            if think_match and last_reasoning_idx < last_user_idx:
                messages.append({"role": "assistant_reasoning", "content": think_match.group(1).strip()})

        output = client.completion.split('</think>')[-1].strip() if "</think>" in client.completion else client.completion.strip()
...
def finalize_with_error(key):
    ...
    output = _extract_output(client) or "Error occurred - no valid output"
...
def process_lrm_reasoning(key):
    ...
    if not client:
        ...
        return False
    try:
        input_msgs = [{"role": "system", "content": NORMAL_SYSTEM_PROMPT}]
        for m in messages:
            if m['role'] == 'user':
        ...
        if client.completion:
            input_msgs.append({"role": "assistant", "content": client.completion})

        client.chat(messages=input_msgs)

        if "</think>" not in client.completion:
            raise ValueError("Out of the limit length of token. No reasoning process found in the output.")

        think_match = re.search(r"<think>(.*?)</think>", client.completion, re.DOTALL)
        if think_match:
            thinking_content = think_match.group(1).strip()
            messages.append({"role": "assistant_reasoning", "content": thinking_content})

        output = client.completion.split('</think>')[-1].strip()
        item = st.session_state.current_item
        reward, answer, gold = verify_answer(output, item)

        if reward is True:
            messages.append({"role": "assistant_output", "content": output})
            ms_set(key, "result", {"reward": reward, "answer": answer, "gold": gold, "output": output, "auto_completed": True})
            ms_set(key, "completed", True)
...
def _send_lrm_feedback(key, feedback):
    ...
    if step == "REASONING":
        ...
        with st.spinner("Processing..."):
            if model_type == "pir":
                ...
                process_lrm_reasoning(key)
            st.rerun()

    elif step == "WAITING_USER_INPUT":
        if model_type == "lrm" and ms(key, "current_output"):
            with st.chat_message("assistant", avatar="💬"):
            ...
            if st.button("📤 Send", key=f"model_{key}_send_btn", use_container_width=True):
                if user_input:
                    if model_type == "pir":
                        ...
                        _send_lrm_feedback(key, user_input)
        with c2:
            if model_type == "lrm":
        ...
        with col:
            ...
            with st.expander("💬 Chat History", expanded=False):
                for msg in ms(key, "messages"):
                    if msg['role'] != "assistant_reasoning":  # 不在前端显示 reasoning 过程
